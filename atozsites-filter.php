<?php
/**
 * Plugin Name:     Atozsites Filter
 * Plugin Slug:     Atozsites-Search-Filter
 * Plugin Nick:     Atozsites
 * Plugin URI:     https://github.com/atozsites/search-filter
 * Description:     Search and Filtering system for Pages, Posts, Categories, Tags and Taxonomies.
 * Author:          Atozsites
 * Author URI:      http://atozsites.com
 * Text Domain:     atozsites-Search-Filter
 * Version:         1.0
 */
...
*/
if (!defined('Atozsites_VERSION_NUM'))
    define('Atozsites_VERSION_NUM', '1.0');

if (!defined('Atozsites_THEME_DIR'))
    define('Atozsites_THEME_DIR', ABSPATH . 'wp-content/themes/' . get_template());

if (!defined('Atozsites_PLUGIN_NAME'))
    define('Atozsites_PLUGIN_NAME', trim(dirname(plugin_basename(__FILE__)), '/'));

if (!defined('Atozsites_PLUGIN_DIR'))
    define('Atozsites_PLUGIN_DIR', WP_PLUGIN_DIR . '/' . Atozsites_PLUGIN_NAME);

if (!defined('Atozsites_PLUGIN_URL'))
    define('Atozsites_PLUGIN_URL', WP_PLUGIN_URL . '/' . Atozsites_PLUGIN_NAME);

if (!defined('Atozsites_BASENAME'))
    define('Atozsites_BASENAME', plugin_basename(__FILE__));

if (!defined('Atozsites_VERSION_KEY'))
    define('Atozsites_VERSION_KEY', 'Atozsites_version');
...
add_option(Atozsites_VERSION_KEY, Atozsites_VERSION_NUM);

/*
* Set up Plugin Globals
*/
if ( ! class_exists( 'Atozsites' ) )
{
	class Atozsites
	{
		...
		public function __construct()
		{
			...
			add_shortcode('Atozsites', array($this, 'shortcode'));
		...
		{
			wp_enqueue_style( 'Atozsites', Atozsites_PLUGIN_URL . '/style.css', false, 1.0, 'all' );
		}
		public function of_enqueue_admin_ss($hook)
		{
			if( 'toplevel_page_Atozsites-settings' == $hook )
			{
				wp_enqueue_style( 'of_syntax_style', Atozsites_PLUGIN_URL.'/admin/github.css', false, 1.0, 'all' ); //more highlight styles http://softwaremaniacs.org/media/soft/highlight/test.html
				wp_enqueue_style( 'of_style', Atozsites_PLUGIN_URL.'/admin/style.css', false, 1.0, 'all' );
				//wp_enqueue_script( 'of_syntax_script', Atozsites_PLUGIN_URL.'/admin/syntax.highlight.min.js' );
			}
		}
		...
		*/
		...
		public function get_search_filter_form($submitlabel, $search_placeholder, $fields, $types, $labels, $hierarchical, $hide_empty, $show_count, $post_types, $order_by, $order_dir, $operators, $all_items_labels, $empty_search_url, $add_search_param, $class)
		{
			...
			$returnvar .= '
				<form action="" method="post" class="Atozsites'.$addclass.'">
					<div>';
...
// classes
require_once(Atozsites_PLUGIN_DIR."/of-list-table.php");
require_once(Atozsites_PLUGIN_DIR."/of-taxonomy-walker.php");

// admin screens & plugin mods
require_once(Atozsites_PLUGIN_DIR."/of-admin.php");
